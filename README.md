...
*   **Global Terms & Privacy Gatekeeper [NEW]**: Enforces active consent via a glassmorphic welcome terms modal on initial load. Users are blocked from accessing or interacting with the dashboard until they read the terms and check the agreement box to accept, with their selection stored in `localStorage` to avoid repeatedly prompting them.
